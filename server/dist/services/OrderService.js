...
const MONEY_SCALE = 8;
const TEST_PLATFORM_FEE_RATE = new client_1.Prisma.Decimal("0.05");
class OrderService {
    prismaClient;
    ...
    async confirmOrder(input) {
        ...
        return this.prismaClient.$transaction(async (transactionClient) => {
            ...
            if (existingOrder.status !== client_1.OrderStatus.PENDING) {
                throw new Error(`Order ${orderId} cannot be confirmed from status ${existingOrder.status}.`);
            }
    ...
    async completeOrder(input) {
        ...
        return this.prismaClient.$transaction(async (transactionClient) => {
            ...
            if (order.status !== client_1.OrderStatus.PAID) {
                throw new Error(`Order ${orderId} cannot be completed from status ${order.status}.`);
            }
